...
#    除外する。関連サイト（新大久保グルメ）がそれで、移転前の置き場では
#    ない ── 名指ししないと「旧ホストが残っています」で配置が止まる。
#    api.kstudy101.jp を書いた時にビルドが落ちたのと同じ形。
RELATED_HOSTS="sinokubo.pages.dev"
