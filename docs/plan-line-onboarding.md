...
## 6. 대운 반영 결정 시 함께 개정할 것 (2026-08-06 추가 지시 2)

gender 를 대운 계산에 **반영하는 날**, privacy 의 두 곳이 함께 개정 대상이다:
① 「現時点では保存のみで、運勢の計算には用いていません」(제2항 문단) — 사전 개정
고지 약속대로 반영 전에 고칠 것. ② **표의 「答えないのままでもすべての機能を
ご利用いただけます」** — 반영 후에는 무회답자와 회답자 사이에 대운 정밀도 차이가
생기므로 이 문장이 오해를 만든다 (「기능은 전부 쓰되 대운 정밀도가 다르다」로).
