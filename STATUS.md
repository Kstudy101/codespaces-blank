...
최종 갱신: 2026-08-09 (유입 2모델·관련사이트 메뉴·LINE 침묵 추적 — 13커밋 배포 완료 / **미해결 1건 §0-★**)
...
- **유입이 2개가 되었다** (2026-08-09) — ①사이트 진단→연동 ②사이트 배너→LINE 직행.
  양쪽이 **문자 단위로 같은** 웰컴을 받는다. 친구추가 1통째는 **LINE 인사말**이
  보낸다 — 우리 서버가 죽어도 나가도록 옮겼다(§8-11)
...
**권장 진행 순서 (한 줄).**
**§0-★ 의 LINE 침묵을 `stderr.log` 로 확정** → 대표 콘솔 3건(§0-☆) → contact.html 개명 범위 결정(§0-☆) → E1(복습 퀴즈) 조건 도래 시. C4는 재개 시 [live-check-c4](docs/live-check-c4-sales-open.md).

### §0-★ **미해결** — 배너로 들어와도 아무 반응이 없다 (2026-08-09)

증상: 사이트 배너 → LINE → **아무 메시지도 오지 않음.** 원인을 넷까지 벗겨냈고
셋은 고쳤지만(아래 §9 참조), **아직 침묵이 남아 있습니다.**

지금까지 확인된 것:

| | |
|---|---|
| 배너 URL | 정상 (`lin.ee/SKZtS5k` → `@798oqmjl`, 301 확인) |
| 웹훅 배관 | **살아 있음** — 대화창에 「あ」를 치면 2통이 온다 (message 경로) |
| `follow` 처리 | **코드는 정상** — 3가지 상태를 재현해 전부 1통 반환 확인 |
| 인사말 | 대표가 콘솔에 등록 완료 (2026-08-09) |
| 차단→재추가 | 대표가 실시 |

→ 남은 것은 **`follow` 이벤트가 서버에 닿았는가**뿐. 서버는 모든 이벤트를
로그에 남긴다([app.mjs:167](server/app.mjs#L167)).

```bash
tail -n 100 ~/kstudy101-line/stderr.log     # 친구추가 직후에
```

`event {"type":"follow",…}` 가 있으면 우리는 보냈다(전송 단계 문제).
없으면 `follow` 가 도달하지 않은 것(LINE 콘솔 쪽).

> **관련 결함 — 아직 안 고침:** `handlers/follow.mjs` 의 회신부가
> `catch { }` 로 **실패를 통째로 삼킨다.** 주석의 「내일 아침 안내로 줍는다」는
> 이름 없는 사람에겐 거짓 — 배신 대상이 아니라 영원히 아무것도 안 온다.
> 오늘 추적이 길어진 이유의 절반. 여기에 원인을 남기도록 고칠 것을 권함(미승인).

### §0-☆ 대표 실기 대기 (2026-08-09)

| # | 위치 | 할 일 | 안 하면 |
|---|---|---|---|
| 1 | cPanel → Environment variables | `LINE_ADD_FRIEND_URL` 이 옛 `line.me/R/ti/p/@kstudy101` 이면 `https://lin.ee/SKZtS5k` 로 교체 | **Stripe 결제 복귀가 404** ([checkout.mjs:378](server/lib/handlers/checkout.mjs#L378)) |
...
「名前からはじめるハングル勉強」 그대로. 전면 개명이면 `tokushoho.html`(특정상거래법
표기·11곳)과 `privacy.html`(10곳)도 같은 커밋에서 고쳐야 함. 결정 전까지 커밋하지 않음.

**§0-☆-b 루트 `package.json`.** 내용이 `"@google/design.md": "^0.4.0"` 하나 —
이 프로젝트와 무관하다. `verify-server` 가 「루트에 package.json 은 없는 채로」를
검사하므로, 커밋되면 사이트·서버 배포가 둘 다 멈춘다. **`git add -A` 금지.**
...
| ID | 작업 | 담당 | 근거 |
|---|---|---|---|
| **★-LOG** | 친구추가 직후 `tail -n 100 ~/kstudy101-line/stderr.log` — `follow` 이벤트 유무 확인 | 대표 | §0-★. 이것 없이는 더 좁힐 수 없다 |
...
| ID | 작업 | 담당 | 근거 |
|---|---|---|---|
| **★-CATCH** | `follow.mjs` 의 회신 `catch { }` 에 원인을 남기기 | 개발 | §0-★ 말미. **미승인** |
| **☆-RETRY** | 배포 재시도 1회 자동화 — UAPI 5회 + `ssh-keyscan` 1회 발생 | 개발 | §9.3. **미승인**(계획서부터) |
...
10. **문면을 바꿀 때는 관문도 같은 커밋에서 고치십시오.** 2026-08-09 하루에
    이것으로 배포가 3번 멈췄습니다(`verify-quiz` 의 `/正解/`, `verify-onboarding`
    의 `「1 日目」`·`［受講料］`). 관문이 옛 문자열을 그대로 찾고 있으면
    「고친 사람이 아니라 다음 사람」이 배포에서 막힙니다
11. **웰컴보드는 LINE 관리화면에도 같은 사본이 있습니다**(2026-08-09).
    친구추가 1통째는 LINE の あいさつメッセージ 가 보냅니다 — 우리 서버가 죽어도
    나가게 하려고 옮겼습니다. 대신 **관문이 볼 수 없는 사본**이 하나 생겼습니다.
    약속은 하나뿐: **코드가 정본, 콘솔은 붙여넣기 사본.** 문면을 고치면
    `welcomeMessages` 로 뽑아서 콘솔에 다시 붙여넣으십시오
12. **LINE Login 은 친구로 만들지 않습니다.** 연동(누구인지)과 친구추가는 별개고,
    친구가 아니면 이쪽에서 **1통도 보낼 수 없습니다.** 그래서 연동 URL 에
    `bot_prompt=aggressive` 를 붙여 LINE 자체 「친구추가」 화면을 끼웠습니다.
    Login 채널에 Official Account 가 연결돼 있어야 동작합니다(§0-☆-2)
...
읽는 순서는 아래 표의 **위에서 아래**가 시간순 역순입니다.

| 커밋 | 내용 |
|---|---|
| `8d44bf0` | **인사말을 LINE 쪽으로 + `bot_prompt`** — 친구추가 1통째를 LINE の あいさつメッセージ 가 보내도록 이관(우리 서버가 죽어도 나간다). `handleFollow` 는 다음 질문만. 코스까지 정해진 사람은 우리 쪽 **0통**. 그리고 `authorizeUrl` 에 `bot_prompt=aggressive` — LINE Login 은 「누구인지」만 알려줄 뿐 친구로 만들지 않아, 연동만 하고 친구가 안 된 사람에게는 **1통도 못 보낸다**. verify-webhook 58 |
| `1df2b4c` | 배너에 「友だち追加ですぐスタート」 — 자동 친구추가는 API 에도 없다. 「追加」를 본인이 눌러야 한다는 것을 미리 말한다 |
| `2b108e5` | **막다른 화면 복구** — `users` 행이 없으면 `message`·`postback` 이 **조용히 반환**하고 있었다. LINE 쪽은 친구라 `follow` 가 두 번 다시 안 오므로, 리치메뉴는 뜨는데 무엇을 눌러도 침묵하는 **출구 없는 화면**이 남았다. 접촉해 온 기회에 그릇을 다시 놓고 웰컴 2통. postback 은 누른 버튼의 용건을 버린다 |
| `5a2b4b1` | **죽은 친구추가 URL** — `line.me/R/ti/p/@kstudy101` 은 **404**였다(실제 ID `@798oqmjl`). 같은 문자열이 배너·`pages.mjs` 기본값·`.env.example` 세 곳에 있었고, 그 기본값을 **Stripe 결제 복귀 URL 이 쓴다**. 전부 `lin.ee/SKZtS5k` 로. 배너 색도 사이트 기조색(`--accent`)으로 |
| `fd02b78` | **유입 2 — LINE 직행 배너** (지시서㉓ §1). 豆知識 바 바로 아래. `verify-pages` 가 배너 href 와 `pages.mjs` 기본값을 **대조**한다 — 이 관문이 당일에 위 404 를 잡아냈다 |
| `cecc37a` | **유입 1·2 의 웰컴 통일** (지시서㉓ §0-A) — 이름 유무로 갈리던 분기를 없앰. `welcomeForNameless` → `welcomeBoard`. 관문은 두 유입의 1통째를 **`===` 로** 대조(부분 일치면 한쪽만 고쳐도 통과한다) |
| `1f74ca0` | 코스 선택 화면 개편 — 🟢🟡🔴 로 3덩어리, 한일 병기 머리글. 「このあとすぐ「1日目」」로 실동작과 일치시킴. **선택 전 고지 3줄 삭제**(대표 확정) — 「코스 교체 불가」는 이제 **어디에도 없다**(경위는 checkout.mjs 주석) |
| `b73604b` | 친구추가 웰컴 **2통 중복 제거** — `serviceGuide` 의 「連携できました」가 친구추가만 한 사람에게 나가고 있었다(사실도 아님). `link.mjs`·리치메뉴에는 유지 |
| `8874f19` | 온보딩 문면(대표 편집) + 관문 2곳 정합. `［受講料］`→`下のメニュー` 로 관문이 찾는 대상을 바꿈 |
| `d6cb1ef` | 상세창 제목 `新大久保グルメ-準備中...` |
| `b984287` | 사이드메뉴 **관련사이트** 개칭 + **상세창 구조** — 서랍에서 직접 밖으로 나가지 않고, `<dialog>` 로 URL·설명을 먼저 보여준다. ESC 가 메뉴까지 닫던 것 수정. verify-pages 16→19 |
| `bd17dd6` | **`verify-quiz` 를 확정 문면에 맞춤** — `ad5671c` 가 문면만 바꾸고 관문을 안 고쳐, 사이트·서버 배포가 **3커밋 동안 빨간불**이었다 |
| `9ba8048` | 사이드메뉴 자매사이트 항 추가. `build-site.sh` 의 「구 호스트 잔존」 검사가 `*.pages.dev` 를 전부 걸러 배포를 막던 것도 함께(명시 제외) |

**이 날의 교훈 3개.**
1. **문면을 바꾸면 관문도 같은 커밋에서.** 오늘 이것으로 배포가 3번 멈췄다(`verify-quiz`·`verify-onboarding` 2곳). 각 위치에 경위를 주석으로 남겨 두었다
2. **관문이 사고를 굳히고 있을 수 있다.** 「미등록자는 채점하지 않는다」가 「조용히 아무것도 안 한다」로 구현돼 있었고, 검사 이름이 맞아서 아무도 의심하지 않았다
3. **증상 하나에 원인이 넷이었다.** 「눌러도 아무 일 없음」 아래에 404·죽은 기본값·조용한 반환·`follow` 의존이 겹쳐 있었고, 하나를 벗길 때마다 다음 것이 드러났다. 아직 §0-★ 가 남아 있다
