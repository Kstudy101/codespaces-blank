...
/* ---- 日数制限の一時解除（2026-08-16 대표 지시）-----------------------
   体験の 7 日を使い切った人は、翌朝から配信が止まる（残り = entitled −
   used が 0 以下）。ところが決済は Stripe の審査待ちで開けられない
   （SALES_MODE=test）ので、止まった人には**買う手段が無い**。
   事業転換のあいだ、講座がただ静かに終わるほうが損なので配り続ける。

   ここが唯一の出どころ。読むのは 2 か所だけ:
     ・db/push-daily.mjs  残り 0 でも降りない（＝配る）
     ・db/push-evening.mjs 体験終了の予告を出さない
   後者を忘れると、6 日目の夕方に「明日で体験が終わります」と言った
   翌朝に 8 日目が届く ── 言ったことと起きることが食い違う。

   【日数の消費は止めない】
   advanceDay はそのまま（確保と消費を 1 文で行う不変式・STATUS §8-2）。
   止める版を足すと「進める」関数が 2 本になり、どちらを呼ぶかを
   間違えた日に 1 日ぶんが無料になる。消費し続けるので残りは
   マイナスへ進む ── 無料で配ったぶんが帳簿に残る形で、これが正しい。

   ★ 制限を戻す日（false に直す日）は、その時点で残りが大きく
     マイナスの人が居る。全員がその朝から止まる ── それが
     「制限を戻す」ということ。戻す前に、買える状態（SALES_MODE=open）
     になっていることを先に確かめること。 */
