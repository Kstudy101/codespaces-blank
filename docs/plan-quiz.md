...
> **D3a 초급 + D3b 중·고급** 1〜50 복습 pool 각 **51문**（48+절목30/50/75）·합 **153문** — **서버 seed 완료** (2026-08-08).
...
| 관문 `verify-quiz` 신설 | ~~중급·고급 백필 (원고가 0)~~ → **D3b로 승인 (2026-08-08)** |
| 초급 1~50일 퀴즈 백필 (원고 작업) | `privacy.html` — **저장 항목이 늘지 않으므로 불변** |
| **D3b** 중·고급 1~50일 복습 퀴즈 백필 (원고 작업) | 51〜101 · 절목 발신 코드 (이미 완료) |
...
6. `[x]` 백필 D3a — **초급 1〜50** 48문 추가 + 절목 30/50/75 = **51문** (2026-08-08).
   `gen-review-quiz.mjs` → `merge-quiz.mjs --write` → `seed-content.mjs --check` ✓.
7. `[x]` 백필 D3b — **중·고급 1〜50** 각 48문 + 절목 = **코스당 51문 · 합 153문** (2026-08-08, 대표 승인).
   서버 FTPS·`bash db/run.sh db/seed-content.mjs` · `答え合わせ` 153問 확인 (2026-08-08 종결).
