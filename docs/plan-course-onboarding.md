...
> **2026-08-09 갱신 (지시서25).** 체험이 3일 → **7일**이 되면서, 아래 (다)(라)의
> 유도 시점은 「2일차 저녁」이 아니라 **`TRIAL_UPSELL_DAY`(= `TRIAL_DAYS - 1`) 저녁**
> 입니다. 7일이면 **6일차**. 「N 日目まで進みました」는 이제 상수가 아니라
> **DB 의 실제 `current_day`** 를 넣습니다 — 판정과 표시가 어긋나면 문면이
> 그것을 숨기지 않도록. 아래 블록은 7일 기준의 실물입니다.

**(다) 체험 종료 전날 저녁 결제 유도 — 살 수 있을 때** — **확정**:
...
**(라) 체험 종료 전날 저녁 결제 유도 — 살 수 없을 때** — **신규 확정 (승인 수정 1)**:
