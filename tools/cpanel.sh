...
[ $# -ge 1 ] || { echo "使い方: cpanel.sh Module/function [key=value ...]" >&2; exit 1; }
CALL=$1; shift
...
resp=$(curl -sS -m 60 -G \
  ...
  echo "✗ 接続できません: https://${CP_HOST}:${CP_PORT}/execute/${CALL}" >&2
  exit 1
}

# UAPI は HTTP 200 のまま status:0 で失敗を返す。エラーは errors 配列。
# ここで見ないと、呼んだ側は成功したと思って次へ進む。
