...
/* ---- 差し込み口 ----------------------------------------------------
   はじめは名前だけ。のちに五行と干支を足した ── かつてこの講座が
   「四柱で韓国語を学ぶ」ものだったため。どれも助詞が付くと받침で
   形が変わるので、名前と同じ仕組みをそのまま広げてある。

   ★ 2026-08-16 の事業転換以降、{OHAENG}/{ZODIAC} に値が入る道は
     どこにも無い（値の出どころだったサイト診断が先に消え、運勢も
     廃止した）。原稿もこれらを使えない ── content-check が
     「四柱の無い人が読めない日は入稿できない」で弾く。
     スロットごと外すかは未決（docs/plan-fortune-removal.md D7）。
