...
> **상태: 승인 대기.** 대표님 확정사항(2026-08-04) 반영:
...
**메시지 수 상한 확인** — 최악의 날: 본편 2 + 운세 1 + 기한예고 1 + 복습퀴즈 1 = **5통.
LINE 의 1회 push 상한이 정확히 5**라 통과하지만 여유가 0입니다. 앞으로 아침 배신에
통을 더 붙일 일이 생기면 이 계산부터 다시 해야 합니다 (코드 주석에 남깁니다).
...
1. `[ ]` 003 + migrate.mjs + repo(learning) + content-check
2. `[ ]` render + push-daily + postback
3. `[ ]` verify-quiz 신설, 기존 17종 회귀 확인
4. `[ ]` 관문 목록 4곳 갱신
