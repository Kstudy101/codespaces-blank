...
/* action → 処理。ここに無い action は onUnknown へ落ちる ── 表に足し
   忘れると「押しても無反応」ではなく skipped が返るので、握りつぶした
   ことが呼ぶ側に見える（この分け方は plan-refactor-handlers.md §3）。 */
const ACTIONS = {
  name:      onName,
  fix:       onFix,
  about:     onAbout,
...
/* 生年月日・出生地・性別の旧ボタンは受けるが保存しない。押した人の
   端末には古いリッチメニューが残りうるので、黙殺せず受け止める。 */
...
async function onTrackpick(ctx) {
  const { conn, event, user, action, params, token, send, deliver, push, transact } = ctx;
  const track = params.track;
  if (!isTrack(track)) return { skipped: `未知の track: ${track}`, userId: user.id };
...
export async function handlePostback(conn, event,
  ...
  const ctx = { conn, event, user, action, params, token, send, deliver, push, transact };
  if (DROPPED.includes(action)) return onDropped(ctx);
  return (ACTIONS[action] || onUnknown)(ctx);
}
