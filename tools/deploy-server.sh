...
#
# 【2026-08-05 転換】自動配備に切り替えた（代表の指示・
# .github/workflows/deploy-server.yml が cPanel UAPI で行う。
# README の設計決定ログ参照）。下の「押した人が見届ける」は
# 元の決めごとの記録で、このスクリプトはいまは**代替手段** ──
# Actions が使えない日・Secrets が無い機体のための道。
#
